...
private:
	...
	// function pointers ----------------------------------------------------------------------------------------------
		...
		boost::function<void (const Long& nid, const Long& sensorId, const RadarDefinition& rdef, SensorDefinition& def, SensorStatus& s, const String& distSensorDomainFile, const String& distJammerDomainFile)> _radarInit;
		boost::function<void (const Long& time, const Long& timestep, const Long& nid, const SensorDefinition& def, SensorStatus& status, SensorsStatus& statusList, const SensorOrders& orders, const EnvironmentInteractions& currentEI, EnvironmentInteractions& newEI, SensorTracks& tracks)> _pRadarStep;
	
	
	// interactions ----------------------------------------------------------------------------------------------
	
	void interaction_radarInit(const vd::Time& time) {
		TraceAlways(vu::DateTime::simpleCurrentDate() + " - Radar::interaction_radarInit - request at time : " + time.toString());
		try
		{
			_radarInit(this->NID,this->sensorId,this->radarDef,this->sensorDef,this->status,this->distSensorDomainFilePath,this->distJammerDomainFilePath);
		}
		catch(...)
		{
			TraceAlways(vu::DateTime::simpleCurrentDate() + " - Radar::interaction_radarInit - Error in calling function radarInit at time : " + time.toString());
		}
	}
	void interaction_pRadarStep(const vd::Time& time) {
		TraceAlways(vu::DateTime::simpleCurrentDate() + " - Radar::interaction_pRadarStep - request at time : " + time.toString());
		try
		{
			_pRadarStep(this->time,this->timestep,this->NID,this->sensorDef,this->status,this->statusList,this->ordersList,this->interactions,this->interactions,this->tracks);
		}
		catch(...)
		{
			TraceAlways(vu::DateTime::simpleCurrentDate() + " - Radar::interaction_pRadarStep - Error in calling function pRadarStep at time : " + time.toString());
		}
	}
	...
	bool guard_initializing(const vd::Time& time) const {
		TraceAlways(vu::DateTime::simpleCurrentDate() + " - Radar::guard_initializing - request at time : " + time.toString());
		bool result = true;
		result &= (this->time==0); 
		return result;
	};
	...
	void action_test2 (const vd::Time& time) {
		TraceAlways(vu::DateTime::simpleCurrentDate() + " - Radar::action_test2 - request at time : " + time.toString());
		this->_signal_test2 = true; }
	
	// Transitions -------------------------------------------------------------------------------------------------------
	
	void transition_INIT_to_RUNNING_for_ModelState(const vd::Time& time) {
		TraceAlways(vu::DateTime::simpleCurrentDate() + " - Radar::transition_INIT_to_RUNNING_for_ModelState] - request at time : " + time.toString());
...
public:
	...
	Radar(const vd::DynamicsInit& init, const vd::InitEventList& events) : vd::Dynamics(init, events) {
		TraceAlways(vu::DateTime::simpleCurrentDate() + " - Radar::Radar");
	...
	virtual vd::Time init(const vd::Time& time) {
		TraceAlways(vu::DateTime::simpleCurrentDate() + " - Radar - Init at time : " + time.toString());
	
	_radarInit = (void (*)(const Long& nid, const Long& sensorId, const RadarDefinition& rdef, SensorDefinition& def, SensorStatus& s, const String& distSensorDomainFile, const String& distJammerDomainFile)) _pFuncDirectory->getFunction("radarInit");
	_pRadarStep = (void (*)(const Long& time, const Long& timestep, const Long& nid, const SensorDefinition& def, SensorStatus& status, SensorsStatus& statusList, const SensorOrders& orders, const EnvironmentInteractions& currentEI, EnvironmentInteractions& newEI, SensorTracks& tracks)) _pFuncDirectory->getFunction("pRadarStep");
	...
	virtual void output(const vd::Time& time, vd::ExternalEventList& output) const {
		TraceAlways(vu::DateTime::simpleCurrentDate() + " - Radar - Output at time : " + time.toString());
	...
	virtual void internalTransition(const vd::Time& time) {
		TraceAlways(vu::DateTime::simpleCurrentDate() + " - Radar - internal transition at time : " + time.toString());
		switch(this->_ModelState_state) {
		case INIT:
			transition_INIT_to_RUNNING_for_ModelState(time);
			interaction_radarInit(time);
			
			break;
		case RUNNING:
			interaction_pRadarStep(time);
	...
	virtual void externalTransition(const vd::ExternalEventList& eventList, const vd::Time& time) {
		TraceAlways(vu::DateTime::simpleCurrentDate() + " - Radar - externalTransition at time : " + time.toString());
		//Update input port 
	...
	virtual vv::Value* observation(const vd::ObservationEvent& event) const {
		TraceAlways(vu::DateTime::simpleCurrentDate() + " - Radar - observation");
